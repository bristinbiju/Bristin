...
const HeroSection: React.FC = () => {
  ...
  return (
    <section className="relative h-screen flex flex-col overflow-x-clip w-full">
      {/* Navbar */}
      <FadeIn delay={0} y={-20} className="w-full">
        <div className="flex justify-between items-center w-full px-6 md:px-10 pt-6 md:pt-8">
          <nav className="flex gap-4 sm:gap-6 md:gap-10 text-[#D7E2EA] font-medium uppercase tracking-wider text-sm md:text-lg lg:text-[1.4rem]">
            {[
              ...
              <a key={item.id} href={`#${item.id}`} className="transition-opacity duration-200 hover:opacity-70 whitespace-nowrap">
                {item.label}
              </a>
            ))}
          </nav>
